...
from truck.forms import UserCreateForm, UserLoginForm, CancelForm, UserForgotForm
...
from truck.email import send_reset_email  # 이메일 전송 관련 함수 truckman 프로젝트
from truck.email import get_reset_token
...
def signup():
    ...
    email_value = form.email.data.strip() if form.email.data.strip() != '' else None
        ...
        if error is None:
            ...
            if user.is_admin:  # 관리자인 경우
                return redirect(url_for('admin.manage_users'))
                    #return render_template('shared/admin_base.html', user=user)
            else:
...
def member_modify():
    ...
    form = UserCreateForm(obj=user)  # 사용자 데이터를 사용해 폼 초기화
...
def forgot_password():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        if user:
            ...
            token = get_reset_token(user)  # Use the new function from email.py
            send_reset_email(user, token)  # Pass the token to the email function
